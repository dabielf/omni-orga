...
test('Task editor retains one list, its rows, context and scroll across routed navigation', async () => {
  const store = createDomainStore(database)
  const goal = store.createGoal({ title: 'Editor context', kind: 'ongoing' })
  store.createGoal({ title: 'Editor subgoal', kind: 'ongoing', parentId: goal.id })
  const parent = store.createTask({ title: 'Editor parent', goalIds: [goal.id] })
  store.createTask({ title: 'Editor child', parentId: parent.id })
  for (let i = 0; i < 18; i++) store.createTask({ title: `Editor row ${i}`, goalIds: [goal.id] })
  const completed = store.createTask({ title: 'Editor completed', goalIds: [goal.id] })
  store.completeTask(completed.id)
  const archived = store.createTask({ title: 'Editor archived', goalIds: [goal.id] })
  store.archiveTask(archived.id)
  store.close()

  for (const width of [1280, 390]) {
    await page.setViewportSize({ width, height: 844 })
    for (const [filter, title] of [['', 'Editor row 17'], ['&available=1', 'Editor row 17'], ['&view=completed', 'Editor completed'], ['&view=archived', 'Editor archived']]) {
      const context = `?goal=${goal.id}&ideal=none${filter}`
      await page.goto(`${url}/tasks${context}`)
      if (!filter) {
        await page.getByRole('button', { name: 'Expand Editor parent', exact: true }).click()
        if (width === 390) await page.locator('.task-rail-menu > summary').click()
        await page.getByRole('button', { name: 'Expand Editor context', exact: true }).click()
        if (width === 390) await page.locator('.task-rail-menu > summary').click()
      }
      const link = page.locator('.task-list .task-name').getByText(title, { exact: true })
      await link.scrollIntoViewIfNeeded()
      const row = await link.elementHandle()
      const count = await page.locator('.task-list .task-name').count()
      const scroll = await page.evaluate(() => scrollY)
      if (!filter || filter === '&available=1') assert.ok(scroll > 0)
      await link.click()
      const dialog = page.getByRole('dialog', { name: 'Task', exact: true })
      await dialog.waitFor()
      assert.equal(await page.locator('.task-list').count(), 1)
      assert.equal(await page.locator('.task-list .task-name').count(), count)
      assert.equal(await row.evaluate(node => node.isConnected), true)
      assert.equal(await page.evaluate(() => scrollY), scroll)
      assert.deepEqual(Object.fromEntries(new URL(page.url()).searchParams), Object.fromEntries(new URLSearchParams(context)))
      await row.evaluate(node => node.focus())
      assert.equal(await dialog.evaluate(node => node.contains(document.activeElement)), true)
      await page.keyboard.press('Tab')
      assert.equal(await dialog.evaluate(node => node.contains(document.activeElement)), true)
      if (!filter && width === 1280) {
        const name = dialog.getByRole('textbox', { name: 'Task name', exact: true })
        await name.fill('Editor renamed')
        await name.blur()
        await page.locator('.task-list .task-name').getByText('Editor renamed', { exact: true }).waitFor()
        assert.equal(await row.evaluate(node => node.isConnected), true)
      ...
      assert.equal(await page.evaluate(() => scrollY), scroll)
      if (!filter) {
        assert.equal(await page.locator('.tree-toggle[aria-expanded="true"]').count(), 1)
        assert.equal(await page.locator('.rail-disclosure[aria-label="Collapse Editor context"]').first().getAttribute('aria-expanded'), 'true')
      }
      await link.click()
      await dialog.waitFor()
    ...
    await page.goto(`${url}/tasks/t_missing?goal=${goal.id}&ideal=none`)
    const missing = page.getByRole('dialog', { name: 'Task', exact: true })
    await missing.getByText('Task not found', { exact: true }).waitFor()
    assert.equal(await page.locator('.task-list').count(), 1)
    await page.keyboard.press('Escape')
    await missing.waitFor({ state: 'detached' })
    assert.equal(new URL(page.url()).pathname, '/tasks')
    assert.equal(new URL(page.url()).searchParams.get('goal'), goal.id)
  }
})
