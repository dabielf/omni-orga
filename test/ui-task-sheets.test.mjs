...
test('task sheets retain failed edits, validate links, and keep close available', async () => {
  ...
  await task.getByPlaceholder('Add a URL, one at a time').fill('https://keep-after-close.example')
  ...
  assert.equal(await task.getByPlaceholder('Add a URL, one at a time').inputValue(), 'https://keep-after-close.example')
  fail = false
