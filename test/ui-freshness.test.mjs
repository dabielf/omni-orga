...
test('saved tasks stay fresh on repeated visits and browser history', async () => {
  ...
  await nav('Calendar')
  await page.locator('.cal-grid').getByRole('button', { name: 'Renamed task', exact: true }).click()
  ...
  await nav('Stats')
  await visible(page.getByText('1 of 1 tasks done · 100%', { exact: true }))
  await nav('Today')
  ...
  await nav('Stats')
  await visible(page.getByText('0 of 1 tasks done · 0%', { exact: true }))
  await nav('Tasks')
  ...
  await page.getByRole('button', { name: 'Close', exact: true }).first().click()
  await nav('Calendar')
  assert.equal(await page.getByRole('button', { name: 'Renamed task', exact: true }).count(), 0)
  await nav('Tasks')
